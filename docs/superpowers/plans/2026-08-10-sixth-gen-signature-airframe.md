...
> **The harness is the source of truth, not the code blocks below.** The `scripts/verify-airframe.mjs` snippets pasted into Tasks 1–10 are the state of that file at authoring time. Several checks have since been tightened by verification passes (`camera_framing` now projects the real vertex set rather than the bounding box's eight corners, which include empty air outboard of the sawtooth; `ground_contact` and `engine_ignition` have tighter bands; `tier0_silhouette` asserts the sawtooth coordinates and not just a command count; `hidpi_canvas_fit`, `camera_path_clear`, `halo_visibility`, `climb_attitude` and `planform_presentation` did not exist). **If you are replaying this plan, do not paste a task's harness block over a newer one** — take the live file and add only what is missing.

- **Bounding box stays 6.00 long × 6.20 across.** Every `CAM` standoff distance was sized against it (`redesign-v2.html:2865-2868`). Tolerance ±0.15.
- **`COCKPIT` stays at exactly `(0, 0.30, 1.25)`** (`redesign-v2.html:2870`). Three push-in beats target it.
- **No new clock.** Everything is a pure function of `Flight.state.alt`, with the existing parked idle yaw as the only exception (`redesign-v2.html:2428`).
...
- **No GLB / no external model.** Procedural only (`redesign-v2.html:2404-2408`).
- **Do not touch** the sky ramp, the altitude mapping, or the tier gate. **No `a` (altitude) value in `CAM` or `JET` may change** — that half is absolute, and the seam altitudes 18000 / 22000 / 32000 / 36000 are load-bearing for the reason `redesign-v2.html:2872-2885` gives. The permitted edits outside `buildAircraft()` and the materials block are the two `JET` ground keyframes, the shadow plane size, the `#jet` SVG symbol, the `window.Stage` handle, and the four items below.

  **Corrected 2026-08-10.** The spec's §7 item 1, which this bullet mirrors, used to end "**No other keyframe in either table changes.**" That sentence was false by the time it was read and has been struck in both documents: three further author-approved changes to `CAM` and `JET` have shipped (below). The claim that survives is the narrower one already stated above — no `a` value may change. **Do not restore the stronger sentence**, and do not treat the two ground keyframes as the only permitted `JET` edit.
- **Four edits added and author-approved 2026-08-10**, after verification found defects that could not be fixed inside the bounds above. They are recorded here because this plan is treated as **executable, not historical** (see commit `f631ffc`) — left out, a replay of Tasks 8–10 would silently revert all four. Each is now held by a harness check, so a replay that did revert one would go red rather than ship.
  - **`CAM`, waypoint 3's wide shot only.** `{ a: 36000, ... }` moved from `p:[8.6, 2.6, 11.0]` to `p:[3.0, 8.5, -12.2]` — astern and high. `a`, `fov`, `roll`, `shift`, `look` unchanged, and this is the only keyframe whose `p[0]` or `p[2]` moved. (Three others had `p[1]` raised; that is the separate item below, added later.) **Why:** the nose is at +z and all sixteen original keyframes sat forward of it, so the sawtooth trailing edge — the feature the whole planform was designed around — was never once shown to a Tier-2 reader; it read only in the flat Tier-0 silhouette, which is backwards. 36000 is the only beat that is unoccluded (50000 and 62000 are 25/25 covered by opaque content beds), large, and far enough from its neighbours to reach astern without traversing the airframe. Held by `camera_path_clear`, which exists because `CAM` interpolates position linearly and a lone astern keyframe can be perfectly framed at both ends while the straight line between them passes through the engine bay.
  ...
  - **`JET`, the sign of every non-zero pitch.** Seven keyframes — 8000, 18000, 22000, 32000, 36000, 46000, 62000 — have `r[0]` negated. No `a`, no `p`, no yaw, no roll. **Why:** the nose is at +Z, so a rotation of θ about X sends `(0,0,z)` to `(0, −z·sin θ, z·cos θ)` — **positive `rotation.x` is nose DOWN**. Every value in the table was positive, including the one commented "rotation", which is aviation's word for the nose coming *up*. Measured: at 8,000 ft the nose apex sat at world y −0.779 against a centre trailing point at +0.469, i.e. 12° nose-down at lift-off and tail-high for the whole climb, on a page whose argument is aerospace training. **If you are replaying and the values look wrong to you, run `climb_attitude` before "correcting" them** — it asserts on world positions rather than on the sign of these numbers, precisely because the sign convention is the thing that was misread, and it sweeps the climb in 250 ft steps to catch an error living only in an interpolated span.
  - **`CAM`, `p[1]` (camera height) at three wide beats:** 5000 `3.0 → 3.45`, 8000 `2.4 → 5.40`, 22000 `2.4 → 3.25`. Nothing else — no `p[0]`, `p[2]`, `a`, `look`, `fov`, `roll` or `shift`, at any keyframe. **Why:** this pays for the pitch fix above. The camera table had been implicitly leaning on the nose-down error — a nose-down airframe tips its top surface *toward* a camera in front of it, roughly doubling the visible planform — so correcting the pitch dropped waypoint 1's establishing shot at 8000 to **0.8° off edge-on**, rendering the signature as a flat dark sliver. Presentation is arithmetic (**camera elevation − nose-up pitch**), which is why `p[1]` is the only lever touched: azimuth drives presentation toward zero and extra pitch crosses through edge-on. Delivered 12.11° / 12.07° / 12.16°. 8000 is deliberately short of a full restoration (which would need y ≈ 7.71, close enough to the 18000 transit's 8.0 to read as a crane shot). **36000 is untouched** — the pitch fix improved it, 27.4° → 36.9°, and it is the only beat that shows the sawtooth. Held by `planform_presentation`; note that `camera_framing` **cannot** substitute for it, staying green with an improved margin on a reverted 8000 keyframe.
- **No negatively-scaled clones for mirroring** — a negative scale inverts winding and lights wrong (`redesign-v2.html:2586-2588`). Mirror by generating coordinates.
- **Materials, exact values:** `SKIN` `#5C6675` (roughness .52, metalness .24), `DECK` `#474F5C` (roughness .55, metalness .22), **no `TRIM`** (removed 2026-08-10 — the ruddervators were its only user and are graphite `SKIN` now; see the spec's §6), `GLASS` `#33547D` (colour unchanged, but **`depthWrite: false`** — added and author-approved 2026-08-10; `GLASS` is `transparent` and three.js defaults those to `depthWrite: true`, so the canopy's front surface at z = 1.777 was laying down depth in front of the cockpit halo at z = 1.30 and depth-culling roughly 90% of it. Halos keep `depthTest: true`. See the spec's §6; held by `halo_visibility`), `DARK` `#2E3540` (roughness .68, metalness .3), `GLOW_HOT` `#FFB0A5`, `GLOW_EDGE` `#E8685C`.
...
```js
    ...
    /* depthWrite:false, added 2026-08-10. A transparent material defaults to
       depthWrite TRUE in three.js and transparent objects draw after opaque
       ones, so the canopy's front surface at z = 1.777 was laying down depth
       in front of the cockpit halo at z = 1.30 and depth-rejecting ~90% of it.
       The halos keep depthTest:true — without it the engine glow would draw
       through the fuselage. Held by halo_visibility; do not remove. */
...
The old tube's belly reached y = −0.38; the new belly reaches −0.318 with its bevel, so the parked height drops from −0.95 to −1.03 to keep the same ~0.1 clearance over the ramp. **Change nothing else in either table *in this step*** — the scope of that sentence is this step, not the plan. Three later author-approved edits do change other keyframes (the `CAM` repose, the seven `JET` pitch signs, the three `CAM` camera heights); they are in Global Constraints and must survive a replay of this task.
